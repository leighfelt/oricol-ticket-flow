...
const GITHUB_OWNER = 'craigfelt';
const GITHUB_REPO = 'oricol-ticket-flow-34e64301';
const MIGRATIONS_PATH = 'supabase/migrations';

/**
 * Fetches migration file content from GitHub
 */
async function fetchMigrationFromGitHub(filename: string): Promise<string> {
  const url = `https://raw.githubusercontent.com/${GITHUB_OWNER}/${GITHUB_REPO}/main/${MIGRATIONS_PATH}/${filename}`;
  
  console.log(`Fetching migration from: ${url}`);
  
  const response = await fetch(url);
  
  if (!response.ok) {
    throw new Error(`Failed to fetch migration ${filename}: ${response.status} ${response.statusText}`);
  }
...
/**
 * Gets list of all migration files from the check-migrations function
 */
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Found ${appliedMigrations?.length || 0} already applied migrations`);
    ...
    for (const filename of pendingMigrations) {
      ...
      try {
        ...
        console.log(`  ⬇️  Fetching SQL content...`);
        const sql = await fetchMigrationFromGitHub(filename);
        
        if (!sql || sql.trim().length === 0) {
          throw new Error('Empty migration file');
        }

        console.log(`  ✓ Fetched ${sql.length} characters of SQL`);

        // Execute the migration SQL using exec function
        console.log(`  🔧 Executing migration via exec()...`);
        ...
        if (recordError) {
          // If it's a unique constraint violation, the migration was already applied
          if (recordError.code === '23505') {
            console.log(`  ℹ️  Migration already recorded in schema_migrations`);
          } else {
            throw recordError;
          }
        }
    ...
    console.log(`\n📊 Summary: Applied ${appliedCount}/${pendingMigrations.length} migrations`);
    ...
    if (errors.length > 0) {
      return new Response(
        JSON.stringify({
          appliedCount,
          errors,
          message: `Applied ${appliedCount} migration(s) with ${errors.length} error(s)`,
        }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
