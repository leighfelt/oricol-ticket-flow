...
// Configuration for fetching migrations from GitHub
const GITHUB_REPO = "craigfelt/oricol-ticket-flow-4084ab4c";
const GITHUB_BRANCH = "main";
const MIGRATIONS_PATH = "supabase/migrations";

// List of all migration files (from check-migrations function)
...
/**
 * Fetch migration SQL content from GitHub
 */
async function fetchMigrationFromGitHub(filename: string): Promise<string> {
  const url = `https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/${MIGRATIONS_PATH}/${filename}`;
  
  console.log(`Fetching migration from: ${url}`);
  
  const response = await fetch(url);
  if (!response.ok) {
    throw new Error(`Failed to fetch ${filename} from GitHub: ${response.status} ${response.statusText}`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (migrationsError && migrationsError.code !== 'PGRST116' && migrationsError.code !== 'PGRST204') {
    ...
    const pendingMigrations = ALL_MIGRATION_FILES.filter(
      (filename) => !appliedVersions.has(filename.replace('.sql', ''))
    );

    console.log(`Found ${pendingMigrations.length} pending migrations out of ${ALL_MIGRATION_FILES.length} total`);

    if (pendingMigrations.length === 0) {
      return new Response(
        JSON.stringify({
          appliedCount: 0,
          message: 'No pending migrations to apply. Database is up to date!',
        }),
        {
